...
class View(ttk.Window):
    def __init__(self, controller):
        ...
        self.__stock_menu.add_command(label="Inventário", accelerator="Ctrl+I", command=self.controller.open_stock_window)
        self.bind_all("<Control-i>", lambda e: self.controller.open_stock_window())
        self.__stock_menu.add_command(label="Entrada", accelerator="Ctrl+E", command=self.controller.open_stock_entry_window)
        self.bind_all("<Control-e>", lambda e: self.controller.open_stock_entry_window())
        self.__menu_bar.add_cascade(label="Estoque", menu=self.__stock_menu)

    # rotina de encerramento gracioso
    def shutdown(self):
        if msgbox.yesno("Deseja encerrar a aplicação?", "Sair do $hopkeeper"):
